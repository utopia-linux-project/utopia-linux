...
static void ccw_update_attr(u8 *dst, const u8 *src, int attribute,
				  struct fbcon_display *p)
{
	int offset = (p->cell_height < 10) ? 1 : 2;
	int width = (p->font_height + 7) >> 3;
	int mod = p->font_height % 8;
		     ...
		     int sx, int dy, int dx, int height, int width)
{
	...
	area.sx = sy * p->cell_height;
	area.sy = vyres - ((sx + width) * p->cell_width);
	area.dx = dy * p->cell_height;
	area.dy = vyres - ((dx + width) * p->cell_width);
	area.width = height * p->cell_height;
	area.height  = width * p->cell_width;
		     ...
		     int sx, int height, int width)
{
	...
	region.dx = sy * p->cell_height;
	region.dy = vyres - ((sx + width) * p->cell_width);
	region.height = width * p->cell_width;
	region.width = height * p->cell_height;
	region.rop = ROP_COPY;
				    ...
				    struct fb_image *image, u8 *buf, u8 *dst)
{
	...
	while (cnt--) {
		...
		if (attr) {
			ccw_update_attr(buf, src, attr, p);
			src = buf;
		}
		      ...
		      int fg, int bg)
{
	...
	u32 width = (p->font_height + 7) / 8;
	u32 glyphsize = width * p->font_width;
	...
	image.fg_color = fg;
	image.bg_color = bg;
	image.dx = yy * p->cell_height;
	image.dy = vyres - ((xx + count) * p->cell_width)
		+ (p->cell_width - p->font_width);
	image.width = p->font_height;
	image.depth = 1;
		       ...
		       int fg, int bg)
{
	...
	w = (p->font_height + 7) >> 3;
	src = ops->fontbuffer + (c.glyph * (w * p->font_width));
	...
	if (attribute) {
		...
		dst = kmalloc_array(w, p->font_width, GFP_ATOMIC);
		if (!dst)
			return;
		kfree(ops->cursor_data);
		ops->cursor_data = dst;
		ccw_update_attr(dst, src, attribute, p);
		src = dst;
	}
	...
	if (ops->cursor_state.image.height != p->font_width||
	    ops->cursor_state.image.width != p->font_height ||
	    ops->cursor_reset) {
		ops->cursor_state.image.height = p->font_width;
		ops->cursor_state.image.width = p->font_height;
	...
	dx = y * p->cell_height;
	dy = vyres - ((vc->state.x + 1) * p->cell_width) + pad;
	...
	if (cursor.set & FB_CUR_SETSIZE ||
	    ...
	    ops->cursor_reset) {
		char *tmp, *mask = kmalloc_array(w, p->font_width,
						 GFP_ATOMIC);
		int cur_height, size, i = 0;
		int width = (p->font_width + 7) / 8;

		if (!mask)
			return;

		tmp = kmalloc_array(width, p->font_height, GFP_ATOMIC);
		...
		size = (p->font_height - cur_height) * width;
		while (size--)
			tmp[i++] = 0;
		size = cur_height * width;
		while (size--)
			tmp[i++] = 0xff;
		memset(mask, 0, w * p->font_width);
		rotate_ccw(tmp, mask, p->font_width, p->font_height);
		kfree(tmp);
	}
