...
static void cw_update_attr(u8 *dst, const u8 *src, int attribute,
		     ...
		     int sx, int dy, int dx, int height, int width)
{
	...
	area.sx = vxres - ((sy + height) * p->cell_height);
	area.sy = sx * p->cell_width;
	area.dx = vxres - ((dy + height) * p->cell_height);
	area.dy = dx * p->cell_width;
		     ...
		     int sx, int height, int width)
{
	...
	region.dx = vxres - ((sy + height) * p->cell_height);
	region.dy = sx * p->cell_width;
		      ...
		      int fg, int bg)
{
	...
	image.fg_color = fg;
	image.bg_color = bg;
	image.dx = vxres - ((yy + 1) * p->cell_height);
	image.dy = xx * p->cell_width;
		      ...
		      int fg, int bg)
{
	...
	if (attribute) {
		...
		ops->cursor_data = dst;
		cw_update_attr(dst, src, attribute, p);
		src = dst;
	}
	...
	dx = vxres - ((y * p->cell_height) + p->cell_height);
	dy = vc->state.x * p->font_width;
	...
	if (cursor.set & FB_CUR_SETSIZE ||
		...
		rotate_cw(tmp, mask, p->font_width, p->font_height);
		kfree(tmp);
	}
